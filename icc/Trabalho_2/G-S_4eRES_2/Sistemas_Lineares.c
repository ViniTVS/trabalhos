	...
	*tempo_medio = 0.0;
	...
	{			
		...
		for (i = 1; i <= nx; i++)
		{
			...
			//  jam
			for (int t = 1; t <= ny%4; t++){
				x[k+3+t] = SL->b[k+3+t] - (SL->dig_inf)*x[k+2+t] - (SL->dig_inf_af)*x[k+t-nx+1] - (SL->dig_sup)*x[k+4+t] - (SL->dig_sup_af)*x[k+t+(nx+5)];
				x[k+3+t] = x[k+3+t]/(SL->dig_p);
				// printf("k + 2 + t: %d\n", k + 2 + t);
			}
		}
