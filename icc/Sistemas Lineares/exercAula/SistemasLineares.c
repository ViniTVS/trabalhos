...
{
	real_t produto=0.0;		//soma de residuo parcial
	real_t r=0.0;
	real_t flinst=0.0;
	unsigned int tam=SL->n;

	for (unsigned int i=0;i<tam;i++){	//Percorre X
	
		for (unsigned int j=0;j<tam;j++){    
			produto+= (SL->A[i*tam+j]) * (x[j]); // AX^
		}

		flinst = SL->b[i] - produto;	//flinston = b-Ax
		r+=flinst*flinst;
...
/*
		if (pivotamento != 0){		// pivotamento
			for(unsigned int i=k+1; i < tam; ++i){
				double max = SL->A[];
				printf("A[0]: %g", max);
				if (max < SL->A[k]){
...
*/
		...
		for(unsigned int i=k+1; i < tam; ++i) {
			double m = SL->A[i*tam + k] / SL->A[k*tam + k];		//multiplicador
			// printf("m : %g   ", m);
			SL->A[i*tam + k] = 0.0;								

			for( int j=k+1; j < tam; ++j )
				SL->A[i*tam + j] -= SL->A[k*tam + j] * m;		
	...
	for(int i = tam  ; i >= 0; i--){		// retrosubstituição
		x[i] = SL->b[i] / SL->A[i*tam + i];	// x[i] = b[i] / A[i][i]

		for (int j = i + 1; j < tam; j++){
			x[i] -= (SL->A[i*tam + j] * x[j]) / SL->A[i*tam + i]; // x[i] = b[i] / A[i][i] - (b[j]*x[j] / A[i][i])
		}
		// printf("x[%d]: %g \n", i, x[i]);
